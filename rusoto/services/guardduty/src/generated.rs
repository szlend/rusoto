...
pub struct TOTODetectorIdsNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListDetectorsRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListDetectorsResponse>,
}

impl Iterator for TOTODetectorIdsNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        let res = self.client.list_detectors(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.detector_ids.unwrap_or(Vec::new()));
                new_items.reverse();
...
// Struct for iterating over a paginated API
pub struct TOTOFilterNamesNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListFiltersRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListFiltersResponse>,
}

impl Iterator for TOTOFilterNamesNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_filters(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.filter_names.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
...
// Struct for iterating over a paginated API
pub struct TOTOFindingIdsNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListFindingsRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListFindingsResponse>,
}

impl Iterator for TOTOFindingIdsNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_findings(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.finding_ids.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
...
// Struct for iterating over a paginated API
pub struct TOTOIpSetIdsNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListIPSetsRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListIPSetsResponse>,
}

impl Iterator for TOTOIpSetIdsNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_ip_sets(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.ip_set_ids.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
...
// Struct for iterating over a paginated API
pub struct TOTOInvitationsNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListInvitationsRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListInvitationsResponse>,
}

impl Iterator for TOTOInvitationsNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_invitations(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.invitations.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
...
// Struct for iterating over a paginated API
pub struct TOTOMembersNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListMembersRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListMembersResponse>,
}

impl Iterator for TOTOMembersNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_members(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.members.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
...
// Struct for iterating over a paginated API
pub struct TOTOThreatIntelSetIdsNextTokenIterator {
    // Client for making the request
    client: GuardDutyClient,
    // Parameters for the request
    req: ListThreatIntelSetsRequest,
    // All the items we have downloaded but not tried to yield yet
    buffered_items: Vec<ListThreatIntelSetsResponse>,
}

impl Iterator for TOTOThreatIntelSetIdsNextTokenIterator {
    type Item = TOTO;
    ...
    fn next(&mut self) -> Option<TOTO> {
        ...
        // Request the next batch of items from the API when out of buffered ones
        let res = self.client.list_threat_intel_sets(self.req).sync();
        match res {
            Ok(output) => {
                // Add downloaded items to the buffer
                let mut new_items = &mut (output.threat_intel_set_ids.unwrap_or(Vec::new()));
                new_items.reverse();
                self.buffered_items.append(new_items);
